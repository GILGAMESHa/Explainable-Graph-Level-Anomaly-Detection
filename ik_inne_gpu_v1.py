...
class IK_inne_gpu():
    ...
    def fit(self,
        ...
        # shape=(t, psi)
        self._center_index_list = np.concatenate(
            [np.random.choice(X.shape[0], size=(1, self._psi), replace=False) for _ in range(self._t)], axis=0)
        ...
        for i in range(self._t):
            ...
            s2s = torch.cdist(torch.tensor(sample, dtype=torch.float32, device=self.device),
                              torch.tensor(sample, dtype=torch.float32, device=self.device), p=2)
